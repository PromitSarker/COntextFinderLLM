...
async def query_documents(request: QueryRequest):
    try:
        ...
        if not results["ids"] or len(results["ids"][0]) == 0:
            return QueryResponse(results=[], answer="No search results from context selected")
        ...
        for idx in range(len(results["ids"][0])):
            ...
            # Filter by threshold (0.65 = 65% similarity)
            ...
            # Skip if cleaned content is empty (was junk)
        ...
        # If no results pass the threshold, return "No search results from context selected" with empty results
